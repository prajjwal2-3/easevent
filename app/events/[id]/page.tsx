      ...
      <div className="border-[6px] m-2 xl:m-5 xl:w-8/12 max-w-2xl rounded-2xl flex flex-col  gap-5 p-2 xl:p-5 min-h-[48rem] h-fit">
        ...
        <section className="w-full flex">
          ...
          <section className="w-1/12 relative ">
            <Favtoggle
              id={event.id}
              className={"bg-slate-800 rounded-full xl:my-4"}
            />
          </section>
        </section>
        <section className="xl:m-2 flex flex-col xl:flex-row justify-between gap-6 w-full">
          <section>
            ...
            <p className="flex gap-2 text-sm xl:text-base">
              ...
              {event.startDateTime.toString().slice(0, 10)}
            </p>
            <p className="flex gap-2 text-sm xl:text-base">
              <Calendar size={20} />
              <span className="font-semibold">End date:</span>{" "}
              {event.endDateTime.toString().slice(0, 10)}
            </p>
          </section>
          <section className="flex flex-col">
            ...
            <p className="flex gap-1 items-center text-sm xl:text-base">
              ...
              <span className="font-medium text-slate-600 xl:text-lg">
                {event.isFree ? "Free" : event.price} INR
              </span>
            </p>
          </section>
        </section>
        ...
        <section className="xl:m-2">
          ...
          <div className="flex gap-2 items-center">
            <Avatar>
              {author.photo && <AvatarImage src={author.photo} />}
              <AvatarFallback>
                {author.firstName.slice(0, 2).toUpperCase()}
              </AvatarFallback>
            </Avatar>
            <p className="font-medium text-base xl:text-lg text-slate-800">
              {author.username}
            </p>
          </div>
        </section>
        <section className="xl:m-2">
          ...
          <p className="flex my-4">{event.description}</p>
        </section>
      </div>
    </div>
  );
